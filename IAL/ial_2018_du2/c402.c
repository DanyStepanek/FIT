...
**/
	...
	tBTNodePtr NewItem = NULL;
	
	if((*RootPtr) == NULL){
		NewItem = malloc(sizeof(struct tBTNode));
		if(NewItem == NULL)
			exit(1);
		...
		while(tmpPtr != NULL){
			father = tmpPtr;
			if(tmpPtr->Cont > Content)
				tmpPtr = tmpPtr->LPtr;
			else if(tmpPtr->Cont < Content)
				tmpPtr = tmpPtr->RPtr;
			else
				return;
		}
...
**/
	...
	tStackP StackPtr;
	SInitP(&StackPtr);
	Leftmost_Preorder(RootPtr, &StackPtr);
	tBTNodePtr tmpPtr;
	
	while(!SEmptyP(&StackPtr)){
		tmpPtr = STopPopP(&StackPtr);
		Leftmost_Preorder(tmpPtr->RPtr, &StackPtr);
	}
...
**/
	...
	tStackP StackPtr;
	SInitP(&StackPtr);
	Leftmost_Inorder(RootPtr, &StackPtr);
	...
	while(!SEmptyP(&StackPtr)){
		tmpPtr = STopPopP(&StackPtr);
		BTWorkOut(tmpPtr);
		Leftmost_Inorder(tmpPtr->RPtr, &StackPtr);
	}	
}
...
**/
	...
	Leftmost_Postorder(RootPtr, &StackPtr, &StackBool);
	
	while(!SEmptyP(&StackPtr)){
		tmpPtr = STopPopP(&StackPtr);
		SPushP(&StackPtr, tmpPtr);
		LtoR = STopPopB(&StackBool);
		if(LtoR){
			SPushB(&StackBool, FALSE);
			Leftmost_Postorder(tmpPtr->RPtr, &StackPtr, &StackBool);
		}
		else {
			STopPopP(&StackPtr);
			BTWorkOut(tmpPtr);
...
**/
	...
	do {
		if((*RootPtr) == NULL){
			if(!SEmptyP(&StackPtr))
				(*RootPtr) = STopPopP(&StackPtr);
		}
		else { 
			if((*RootPtr)->RPtr != NULL)
				SPushP(&StackPtr, (*RootPtr)->RPtr);		
			
			tmpPtr = (*RootPtr);
			(*RootPtr) = (*RootPtr)->LPtr;
			free(tmpPtr);
		}
	} while(((*RootPtr) != NULL) || (!SEmptyP(&StackPtr)));
