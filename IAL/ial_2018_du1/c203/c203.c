...
*/
	return ((nextIndex(q->b_index) == q->f_index) ||
			((q->f_index == 0) && (q->b_index == QUEUE_SIZE-1)));
}
...
*/
	if (queueEmpty(q) == 0){			//Fronta nesmi byt prazdna.
		q->f_index++;
		if (q->f_index == QUEUE_SIZE)	//Osetreni preteceni pole.
			q->f_index = 0;
	}
	else
...
*/
	if (queueFull(q) == 0){
		q->arr[q->b_index] = c;
		if (nextIndex(q->b_index) == QUEUE_SIZE)
			q->b_index = 0;
		else
			q->b_index++;
	}
