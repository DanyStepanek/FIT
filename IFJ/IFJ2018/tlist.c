...
Tlist * init_Tlist(){
	Tlist *t = malloc(sizeof(struct Tlist));			//alokace mista pro ukazatel Tlist
	t->arr = malloc(sizeof(struct token)*LIST_SIZE);	//alokace mista pro ukazatel *Token pro pole o velikosti LIST_SIZE
...
int insertNext_Tlist(Tlist* t, char *c, int size, int type){
	...
	if(t != NULL && c != NULL){			// kontrola platnosti vstupnich ukazatelu
		...
		if(token != NULL){
			...
			if(fill_Token(t->arr[t->top], c, size, type) == 0){ // naplneni a kontrola spravneho vlozeni 
				return 0;
			} else{
				return -1;
			}
